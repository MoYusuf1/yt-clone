...
export async function fetchVideos(query: string, maxResults: number) {
  try {
    const { data } = await axios.get(
      `${BASE_URL}/search?key=${API_KEY}&q={query}&order=date&maxResults=${maxResults}&type=video&part=snippet`
    );
